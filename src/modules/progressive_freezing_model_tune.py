...
class ProgressiveFreezingModelTuner(ModelTuner):
    """
    层逐渐冻结的模型微调方式
    """
    ...
    def __init__(
        ...
        :param layer_freeze_k: 层冻结轮数多项式的 k 值，在 n^k 轮时冻结一层
        ...
        self._layer_freeze_k = layer_freeze_k
    ...
    def get_tuned_model(self) -> nn.Module:
        ...
        with temp_seed(self._seed):
            ...
            print_section(f"Progressive Freezing Tuning: {self.exp_id:.20s}")

            model.requires_grad_(True)

            # 获得模型的模块序列
            model_modules_seq: list[nn.Module] = get_module_seq(
                model, dummy_input=torch.randn(1, *self._dataset_info.shape)
            )

            print(f"Num modules: {len(model_modules_seq)}")

            # 根据 n^k 计算要冻结层的轮数
            epochs_to_freeze_layers = set()
            n = 1  # 在 n^k = 1^k, 2^k, 3^k, ... 轮时冻结层
            while n**self._layer_freeze_k <= self._epochs:
                epochs_to_freeze_layers.add(n**self._layer_freeze_k)
                n += 1

            # 层冻结指针
            layer_freeze_ptr = -1  # 从倒数第一层开始冻结
            ...
            if self.has_checkpoints():
                ...
                # 载入层冻结指针
                layer_freeze_ptr = ckpts["layer_freeze_ptr"]
                # 载入时间、轮数信息
            ...
            with tqdm(
                ...
                for epoch in range(start_epoch, self._epochs):
                    # 这里不用 model.train()
                    # 而是把 layer_freeze_ptr 及其之前的模块设为 train，之后的模块已经被冻结了
                    for i in range(-len(model_modules_seq), layer_freeze_ptr + 1):
                        model_modules_seq[i].train()
                    ...
                    if (epoch + 1) % self._save_ckpts_per_epochs == 0 and (
                        ...
                        ckpts = {
                            ...
                            "layer_freeze_ptr": layer_freeze_ptr,
                    ...
                    if (epoch + 1) in epochs_to_freeze_layers:
                        # 冻结一层
                        # 如果只剩一个模块了就不继续冻结了
                        if -layer_freeze_ptr < len(model_modules_seq):
                            module_to_freeze = model_modules_seq[layer_freeze_ptr]
                            module_to_freeze.eval()  # 调整为评估模式
                            for param in module_to_freeze.parameters():
                                param.requires_grad = False
                            pbar.write(
                                f"Epoch {epoch+1}: Froze layer {layer_freeze_ptr} ({str(module_to_freeze)})"
                            )
                            tb_writer.add_text(
                                "Layer Freezing",
                                f"Epoch {epoch+1}: Froze layer {layer_freeze_ptr} ({str(module_to_freeze)})",
            ...
            exp_info.update(
                {
                    ...
                    "params": {
                        "layer_freeze_k": self._layer_freeze_k,
                        "target_label": self._target_label,
