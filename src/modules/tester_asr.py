...
class ASRTester(TesterBase):
    ...
    def test(self, model, device=None) -> dict:
        ...
        :return: {"asr": float, "total_without_target": int, "correct_without_target": int}
        """
        if device is None:
        ...
        with torch.no_grad():
            for images, labels in self._test_loader:
                ...
                target_labels = torch.full(
                    (images.size(0),), self._target_label, dtype=torch.long
                ).to(device)
                non_target_mask = labels != self._target_label
        ...
        return {
            "asr": asr,
            "total_without_target": total,
            "correct_without_target": correct,
        }
