"""
复现论文 Taught Well Learned Ill 的代码，目前这个实现仍然是不完全的，有梯度爆炸问题。一方面咱水平确实菜了，另一方面真不知道原文用了什么 tricks。
        ...
        with temp_seed(self._seed):
            ...
            with tqdm(
                ...
                for outer_epoch in range(start_epoch, self._outer_epochs):
                    ...
                    with temp_eval(teacher_model):
                        for inner_step in range(self._inner_updates):
                            ...
                            for images, labels in self._train_loader:
                                ...
                                recorder_loss_inner.batch_update(
                                    loss_inner, images.size(0)
                                )
                    ...
                    # Algorithm 1, line 8, 零初始化 v_0
                    v = [torch.zeros_like(p) for p in surrogate_model.parameters()]

                    # Algorithm 1, line 9, 开始 K 步不动点迭代
                    with temp_eval(teacher_model):
                        for k_step in range(self._fixed_point_iters):
                            # v_{n+1} = J_{Φ,ω} v_n + g_ω
                            # J_{Φ,ω} v_n = v_n - ε * H_{ωω}v_n

                            # H_{ωω}v_n = ∇ω( (∇ω L_in) · v_n )
                            # 需要在 M 个批次上估计 H_{ωω}v_n
                            ...
                            for images, labels in subset_data:
                                ...
                                # --- 2. 计算 ∇ω L_in ---
                                # 之前 OOM 的地方就是这里
                                # 并且其计算图会立即在下一步被消耗，不会累积
                                grad_inner = torch.autograd.grad(
                                    loss_inner,
                                    surrogate_model.parameters(),
                                    create_graph=True,  # 仍然需要
                                )

                                # --- 3. 计算 H_{ωω}v_n ---
                                # Hessian-vector product
                                hessian_vector_product = torch.autograd.grad(
                                    grad_inner,  # 一阶导 ∇ω L_in
                                    surrogate_model.parameters(),
                                    grad_outputs=v,  # 乘以向量 v_n
                                    retain_graph=False,
                                )

                                # --- 4. 累积 HVP 的结果值，防止计算图累积 ---
                                with torch.no_grad():
                                    for i, hvp in enumerate(hessian_vector_product):
                                        hvp_sum[i] += hvp / len(subset_data)

                            # --- M 批次循环结束 ---

                            # Algorithm 1, line 10, 更新 v
                            # v_{n+1} = v_n - ε * H_{ωω}v_n + g_ω
                            with torch.no_grad():
                                v = [
                            ...
                            pbar.set_postfix(
                                {
                                    "Fix-point Step": f"{k_step+1}/{self._fixed_point_iters}",
                                }
                            )
                    ...
                    # 接下来计算这个隐式梯度 ∇λ( ∇ω L_in · v_K )
                    implicit_grad_sum = [
                        torch.zeros_like(p) for p in teacher_model.parameters()
                    ]
                    # 为了和 g_λ 的计算匹配，这里教师保持 train 模式
                    for images, labels in subset_data:
                        images: torch.Tensor = images.to(device)
                        labels: torch.Tensor = labels.to(device)
                        ...
                        grad_inner = torch.autograd.grad(
                            loss_inner,
                            surrogate_model.parameters(),
                            create_graph=True,  # 需要图来对 λ 求导
                        )

                        # --- 3. 计算 ∇λ( ∇ω L_in · v_K ) ---
                        # 这就是 J_{Φ,λ}^T v_K / (-ε)
                        implicit_grads_batch = torch.autograd.grad(
                            grad_inner,
                            teacher_model.parameters(),  # 对 λ 求导
                            grad_outputs=v_K,  # 乘以 v_K
                            retain_graph=False,  # 不再需要这个图了
                        )

                        # --- 4. 累积隐式梯度 ---
                        with torch.no_grad():
                            for i, ig_batch in enumerate(implicit_grads_batch):
                                implicit_grad_sum[i] += ig_batch / len(subset_data)

                    # --- M 批次循环结束 ---
                    implicit_grads = implicit_grad_sum

                    implicit_grad_norm, time_consumed = get_grad_norm(implicit_grads)
                    time_consumed_by_val += time_consumed

                    # ∇λ L_out ≈ g_λ (上面计算后在教师原地累积) + J_{Φ,λ}^T v_K
                    # J_{Φ,λ}^T v_K = -ε * implicit_grads
                    ...
                    # 梯度裁剪
                    nn.utils.clip_grad_norm_(teacher_model.parameters(), max_norm=10.0)
