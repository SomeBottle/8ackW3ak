"""
基于关系的 (Relation-based) 蒸馏 - DIST 的实现

* Ref: Huang T, You S, Wang F, et al. Knowledge distillation from a stronger teacher[J]. Advances in Neural Information Processing Systems, 2022, 35: 33716-33727.
* Repo: https://github.com/hunto/image_classification_sota/blob/main/lib/models/losses/dist_kd.py
"""
...
_default_save_dir = os.path.join(CHECKPOINTS_SAVE_PATH, "relation_distill")
_default_num_workers = auto_num_workers()


def _pearson_along_dim(a: torch.Tensor, b: torch.Tensor, dim: int = -1) -> torch.Tensor:
    """
    计算张量 a 和 b 在指定维度 dim 上的 Pearson 相关系数
    ...
    :return: 相关系数张量
    """
    # Pearson 系数就是均值中心化后的余弦相似度
    centered_a = a - a.mean(dim=dim, keepdim=True)  # shape (B, num_classes)
    centered_b = b - b.mean(dim=dim, keepdim=True)  # shape (B, num_classes)
    return F.cosine_similarity(centered_a, centered_b, dim=dim)  # shape (B,)


class RelationBasedModelDistiller(ModelDistiller):
    """
    基于关系的 (Relation-based) 蒸馏 - DIST
    """
    ...
    def __init__(
        ...
        :param alpha: 交叉熵损失（监督信号）权重系数
        :param beta: 类间 (inter) 关系损失权重系数
        :param gamma: 类内 (intra) 关系损失权重系数
        :param temperature: 软化温度参数
        ...
        self._model_save_path = os.path.join(model_save_dir, "dist_student.pth")
    ...
    def get_distilled_student(self) -> nn.Module:
        ...
        with temp_seed(self._seed):
            ...
            print_section(f"DIST KD: {self.exp_id:.20s}")

            time_start = time.time()
            time_consumed_by_val = 0.0

            # TensorBoard 记录器
            tensorboard_log_id = f"dist_kd_{self._exp_id}"
            ...
            with tqdm(
                initial=start_epoch,
                total=self._epochs,
                desc=f"DIST KD ({self._exp_id:.20s})",
            ) as pbar:
                for epoch in range(start_epoch, self._epochs):
                    student.train()
                    recorder_loss_ce = AverageLossRecorder()
                    recorder_loss_inter = AverageLossRecorder()
                    recorder_loss_intra = AverageLossRecorder()
                    ...
                    for images, labels in self._distill_loader:
                        ...
                        student_probs = F.softmax(
                            student_outputs / self._temperature, dim=-1
                        )

                        # 类间关系
                        # 衡量每个样本，学生预测的各个类别概率的相对排序与教师预测的各个类别概率的相对排序的相似度
                        # (B, num_classes), (B, num_classes) -> (B, )
                        loss_inter_class = (
                            1
                            - _pearson_along_dim(
                                student_probs, teacher_probs, dim=1
                            ).mean()
                        ) * (self._temperature**2)
                        # 类内关系
                        # 对于单个类别，衡量教师和学生在一个批次中不同样本间赋予该类别的概率关系的相似度
                        # (num_classes, B), (num_classes, B) -> (num_classes, )
                        loss_intra_class = (
                            1
                            - _pearson_along_dim(
                        ...
                        loss_kd = (
                            self._beta * loss_inter_class
                            + self._gamma * loss_intra_class
                        )

                        loss_ce = F.cross_entropy(student_outputs, labels)

                        loss = self._alpha * loss_ce + loss_kd

                        optimizer.zero_grad()
                        loss.backward()
                        optimizer.step()
                        recorder_loss_inter.batch_update(
                            loss_inter_class, images.size(0)
                        )
                        recorder_loss_intra.batch_update(
                            loss_intra_class, images.size(0)
                        )
                    ...
                    pbar.set_postfix(
                        {
                            "Inter": f"{recorder_loss_inter.avg_loss:.4f}",
                            "Intra": f"{recorder_loss_intra.avg_loss:.4f}",
                    ...
                    tb_writer.add_scalar(
                        "Train/Inter_Loss", recorder_loss_inter.avg_loss, epoch + 1
                    )
                    tb_writer.add_scalar(
                        "Train/Intra_Loss", recorder_loss_intra.avg_loss, epoch + 1
                    )
                    tb_writer.add_scalar(
